...
export class AiService implements OnModuleInit {
  ...
  async applyImport(
    ...
    // 纠正即学习（few-shot 自进化）：用户复核时把某条草稿的分类改了的，
    // 记一条「商户 → 新分类」AiCorrection，下次解析时注入 prompt。
    // 原始 categoryId 取自导入记录的 draftsJson（明文草稿，apply 成功后才清除）：
    // 按 (merchantHash, amount, date) 匹配回原始草稿；merchantHash 缺失时退 (amount, date)。
    try {
      const drafts: AiDraft[] = item.draftsJson
        ? JSON.parse(item.draftsJson)
        : [];
      if (drafts.length) {
        const amtKey = (a: number) => Number(a).toFixed(2);
        const dateKey = (d: string) => new Date(d).toISOString();
        const byFull = new Map<string, AiDraft>();
        const byAmtDate = new Map<string, AiDraft>();
        for (const d of drafts) {
          const ad = `${amtKey(d.amount)}|${dateKey(d.date)}`;
          if (!byAmtDate.has(ad)) byAmtDate.set(ad, d);
          if (d.merchantHash) {
            const full = `${d.merchantHash}|${ad}`;
            if (!byFull.has(full)) byFull.set(full, d);
          }
        }
        const learned = new Set<string>();
        for (const b of bills) {
          const ad = `${amtKey(b.amount)}|${dateKey(b.date)}`;
          const draft = b.merchantHash
            ? (byFull.get(`${b.merchantHash}|${ad}`) ?? byAmtDate.get(ad))
            : byAmtDate.get(ad);
          if (!draft || draft.categoryId === b.categoryId) continue;
          const merchantKey = normalizeMerchantKey(draft.counterparty);
          if (!merchantKey || learned.has(merchantKey)) continue;
          learned.add(merchantKey);
          await this.prisma.aiCorrection.upsert({
            where: {
              ledgerId_merchantKey: {
                ledgerId: item.ledgerId,
                merchantKey,
              },
            },
        ...
        if (learned.size) {
          this.logger.log(
            `记录 ${learned.size} 条分类纠正（few-shot 学习，下次解析生效）`,
          );
        }
      }
    } catch (e) {
      this.logger.warn(`记录分类纠正失败（忽略，不影响入库）: ${e}`);
    }
  ...
  private async _process(id: string, buf: Buffer) {
    try {
      ...
      // 2b) 拉该账本最近的分类纠正（few-shot 自进化）：用户复核时改过的
      //     「商户 → 分类」，解析时注入 prompt，越用越准
      const aiCorrections = await this.prisma.aiCorrection.findMany({
      ...
      const drafts = await this._askLlm(model, extracted, cats, aiCorrections);
  ...
  private async _askLlm(
    ...
    if (extracted.kind === 'image') {
      return this._callLlmOnce(model, extracted, categories, corrections);
    }
    ...
    const tasks = labeled.map(async (label, idx) => {
      ...
      return this._callLlmOnce(model, chunkInput, categories, corrections);
    });
  ...
  private async _callLlmOnce(
    ...
    // few-shot 自进化：挑出与该段文本相关的用户历史纠正（商户 → 分类），
    // 注入到分类枚举说明之后，让 LLM 对该账本的用户偏好越用越准。
    const catNameById = new Map(categories.map((c) => [c.id, c.name]));
    const catPathOf = (id: string): string => {
      const c = categories.find((x) => x.id === id);
      if (!c) return id;
      const parent = c.parentId ? catNameById.get(c.parentId) : undefined;
      return parent ? `${parent}>${c.name}` : c.name;
    };
    const fewShotLines =
      extracted.kind === 'text'
        ? buildFewShotLines(
            matchCorrectionsInText(corrections, extracted.content),
            catPathOf,
          )
        : [];
    ...
    const sys: ChatMessage = {
      ...
      content: [
        ...
        '【分类示例】',
        '美团外卖/饿了么/点评团购 → 餐饮>外卖',
        '超市/沃尔玛/盒马/便利店 → 购物>日用百货',
        '滴滴/地铁/公交/火车票/机票/加油 → 交通>打车 或 交通>公交地铁 或 交通>加油充电',
        '房租/物业费/水费/电费/房贷 → 住房>房租 或 住房>物业费 或 住房>房贷',
        '医院/药房/诊所 → 医疗>看病挂号 或 医疗>药品',
        '工资/年终奖/报销/退税 → 工资薪金 或 报销退款 或 其他收入>退税',
        '理发/美甲/奶粉/宠物 → 家庭>理发造型 或 家庭>美容美甲 或 家庭>奶粉尿布 或 家庭>宠物',
        '车险/社保/贷款利息 → 金融保险>车险 或 金融保险>社保公积金',
        '红包/随礼/给父母 → 人情往来>红包 或 人情往来>孝亲赡养',
        '手机充值/宽带缴费 → 通讯>话费 或 通讯>宽带',
        '淘宝/京东/拼多多 → 购物>数码电器 或 购物>日用百货',
  ...
  private _keywordMatch(
    ...
    const map: [string[], string, string?][] = [
      ...
      [['早餐', '早点', '包子', '油条', '豆浆', '豆腐脑', '肠粉', '煎饼果子',
        '馒头', '粥', '鸡蛋灌饼', '午餐', '午饭', '晚餐', '晚饭', '堂食'], '餐饮', '堂食'],
      ...
      [['零食', '薯片', '饼干', '坚果', '果冻', '辣条', '巧克力', '糖果',
        '冰淇淋', '雪糕', '瓜子'], '餐饮', '零食水果'],
      [['烟', '酒', '香烟', '白酒', '啤酒', '红酒', '香烟'], '餐饮', '烟酒'],
      [['请客', '聚餐', '聚会', '宴请', '饭局', 'AA'], '餐饮', '聚餐'],

      // ── 购物 ──
      [['超市', '沃尔玛', '盒马', '山姆', '永辉', '华润', '联华', '大润发',
        '便利店', '全家', '罗森', '7-11', '711', '好德', '美宜佳',
        '屈臣氏', '名创优品', '无印良品', '杂物'], '购物', '日用百货'],
      [['淘宝', '天猫', '京东', '拼多多', '抖音', '快手', '唯品会', '得物',
        '闲鱼', '小红书', '网易严选', '当当', '苏宁', '国美'], '购物', '数码电器'],
      [['数码', '电子', '手机', '电脑', '平板', '耳机', '音箱', '相机',
        '华为', '小米', '苹果', 'vivo', 'oppo', '荣耀', '三星'], '购物', '数码电器'],
      [['家电', '冰箱', '洗衣机', '空调', '电视', '扫地', '吸尘', '风扇', '取暖',
        '净水', '空气炸锅', '烤箱', '微波炉', '电磁炉', '电饭煲', '热水器'], '购物', '家居家装'],
      [['家具', '床垫', '沙发', '桌椅', '灯具', '窗帘', '收纳', '碗', '盘',
        '杯', '锅', '刀具', '菜板', '厨房', '卫浴'], '购物', '家居家装'],
      ...
      [['洗发', '沐浴', '牙膏', '牙刷', '纸巾', '洗衣', '肥皂', '洗手液',
        '卫生巾', '湿巾', '毛巾', '浴巾', '垃圾袋', '拖把', '扫把'], '购物', '日用百货'],
      [['书籍', '书', '文具', '笔', '本', '打印', '复印', '杂志', '报刊',
        'kindle', '电子书', '文具店', '晨光', '无印'], '教育', '书籍文具'],

      // ── 交通（含用车）──
      [['滴滴', '打车', '出租车', '曹操', '首汽', '花小猪', 'T3', '嘀嗒',
        '顺风车', '快车', '专车'], '交通', '打车'],
      [['地铁', '公交', '巴士', 'BRT', '一卡通', '乘车码', 'NFC'], '交通', '公交地铁'],
      [['共享单车', '哈啰', '美团单车', '青桔', '摩拜', '骑行'], '交通', '共享单车'],
      [['火车', '普速', '绿皮', '硬座', '硬卧', '软卧', '12306', '铁路',
        '高铁', '动车', '城际', '复兴号'], '交通', '火车高铁'],
      ...
      [['停车', '泊车', 'ETC', '车库'], '交通', '停车费'],
      [['高速', '过路费', '收费站'], '交通', '过路费'],
      [['车贷', '汽车贷款', '购车贷款', '按揭买车'], '交通', '车贷'],
      [['保养', '汽车维修', '修车', '4S店', '换机油', '机滤',
        '空滤', '轮胎', '电瓶', '刹车片', '火花塞'], '交通', '保养维修'],
      [['洗车', '年检', '审车', '验车', '车管所', '行驶证',
        '驾照', '驾校', '学车', '科目'], '交通', '洗车年检'],
      ...
      [['电影', '影院', '演出', '演唱会', '话剧', '音乐会', '歌剧', 'livehouse',
        '脱口秀', '相声', '展览', '博物馆', 'KTV', '唱歌', '量贩'], '娱乐', '电影演出'],
      [['游戏', 'steam', 'Switch', 'playstation', 'xbox', 'NS',
        '王者荣耀', '原神', '充值', '皮肤', '网游', '手游', '端游'], '娱乐', '游戏'],
      [['会员', '订阅', '视频', '音乐', '网盘', 'VPN', '加速器',
        '腾讯视频', '爱奇艺', '优酷', 'B站', 'QQ音乐', '网易云', 'Apple',
        'iCloud', 'Google', 'Office', 'Notion'], '娱乐', '会员订阅'],
      [['旅游', '度假', '跟团', '自由行', '邮轮',
        '携程旅行', '途牛', '马蜂窝', '穷游'], '娱乐', '旅游度假'],
      [['酒店', '民宿', '美团酒店', '住宿', '宾馆'], '娱乐', '酒店住宿'],
      [['门票', '景点', '景区', '游乐园', '主题公园', '水上乐园', '动物园',
        '海洋馆', '植物园', '博物馆门票', '迪士尼', '环球影城', '长隆'], '娱乐', '旅游度假'],
      [['健身', '健身房', '私教', '瑜伽', '普拉提', 'CrossFit',
        'Keep', '超级猩猩', '乐刻', '威尔仕', '一兆韦德',
        '游泳', '篮球', '足球', '羽毛球', '乒乓球', '网球'], '娱乐', '运动健身'],

      // ── 医疗 ──
      [['医院', '诊所', '门诊', '挂号', '就医', '看病', '急诊', '住院',
        '手术', '化验', '检查', '拍片', 'CT', 'B超', '核磁'], '医疗', '看病挂号'],
      [['药房', '药店', '药品', '买药', '中药', '西药', '处方',
        '感冒药', '退烧药', '止疼药', '消炎药', '医保药'], '医疗', '药品'],
      [['体检', '体检中心', '体检套餐', '入职体检'], '医疗', '体检'],
      [['牙科', '牙医', '口腔', '拔牙', '补牙', '洗牙', '矫正', '种植牙',
        '根管', '牙齿', '牙套'], '医疗', '牙科'],
      [['保健', '养生', '理疗', '针灸', '推拿', '拔罐', '刮痧', '艾灸',
        '保健品', '维生素', '蛋白粉', '鱼油', '钙片'], '医疗', '保健养生'],

      // ── 教育 ──
      [['学费', '培训', '课程', '补习', '家教', '网课', '线上课', '培训班',
        '学而思', '新东方', '猿辅导', '作业帮', '英语', '日语', '韩语'], '教育', '学费培训'],
      [['教材', '书本', '课本', '教辅', '练习册', '试卷'], '教育', '书籍文具'],
      [['考试', '报名', '报名费', '考试费', '考级', '考证', '雅思', '托福',
        '四六级', '考研', '考公', '教资', 'CPA', '司法考试'], '教育', '考试报名'],
      [['兴趣班', '特长班', '钢琴课', '画画课', '舞蹈课', '孩子兴趣'], '教育', '孩子兴趣班'],
      ...
      // ── 人情往来 ──
      [['红包', '发红包', '微信红包', '支付宝红包', '过年红包', '春节红包'], '人情往来', '红包'],
      [['份子钱', '婚礼红包', '结婚红包', '随礼', '随份子', '白事',
        '喜事', '满月', '乔迁', '升学宴'], '人情往来', '随礼份子'],
      [['送礼', '礼物', '礼品', '送人', '过节送礼', '伴手礼',
        '花束', '鲜花'], '人情往来', '送礼'],
      [['请客', '宴请', '招待客户'], '人情往来', '请客招待'],
      [['给父母', '孝敬', '赡养', '生活费给爸', '生活费给妈'], '人情往来', '孝亲赡养'],
      [['给亲戚', '给孩子零花', '补贴家人'], '人情往来', '亲属支持'],

      // ── 家庭（育儿 / 宠物 / 美容）──
      [['奶粉', '尿布', '纸尿裤', '拉拉裤', '湿纸巾', '奶瓶', '奶嘴',
        '吸奶器', '储奶袋', '辅食', '米粉', '果泥', '宝宝零食'], '家庭', '奶粉尿布'],
      [['玩具', '童装', '童鞋', '婴儿车', '安全座椅', '爬行垫',
        '围栏', '婴儿床'], '家庭', '玩具童装'],
      [['早教', '幼儿园', '托班', '亲子', '育儿', '启蒙', '托育'], '家庭', '早教托育'],
      [['宠物粮', '猫粮', '狗粮', '猫罐头', '狗零食', '猫条', '化毛膏',
        '宠物零食', '猫砂', '狗厕所', '宠物医院', '兽药', '驱虫', '绝育',
        '宠物玩具', '猫抓板', '狗绳', '猫窝', '狗窝'], '家庭', '宠物'],
      [['理发', '剪发', '烫发', '染发', '发型', '发廊', '美发店'], '家庭', '理发造型'],
      [['美甲', '美睫', '美容院', '皮肤管理', '水光针', '按摩', 'SPA',
        '足浴', '足疗'], '家庭', '美容美甲'],

      // ── 金融保险 ──
      [['社保', '医保', '养老保险', '失业保险', '工伤保险', '生育保险',
        '公积金', '五险一金', '灵活就业', '城乡居民医保'], '金融保险', '社保公积金'],
      [['商业保险', '重疾险', '医疗险', '寿险', '意外险', '年金险',
        '万能险', '投连险', '香港保险', '平安', '人寿', '太平洋'], '金融保险', '人身保险'],
      [['车险', '交强险', '三者险', '车损险', '商业车险', '汽车保险'], '金融保险', '车险'],
      [['贷款利息', '信用卡利息', '利息支出', '罚息'], '金融保险', '贷款利息'],
      [['消费贷', '信用贷', '花呗分期', '借呗', '分期付款'], '金融保险', '消费贷款'],
      [['手续费', '提现费', '转账费', '汇款费', '服务费'], '金融保险', '手续费'],

      // ── 其他支出 ──
      [['快递', '物流', '运费', '邮费', '顺丰', '圆通', '中通', '韵达'], '其他支出', '快递物流'],
      [['捐赠', '捐款', '公益', '慈善'], '其他支出', '捐赠公益'],
      [['罚款', '罚单', '税费', '滞纳金'], '其他支出', '税费罚款'],

      // ── 收入 ──
      [['工资', '薪资', '薪酬', '基本工资', '绩效', '岗位工资',
        '税后', '税前', '发薪', '薪金', '年终奖', '奖金', '十三薪'], '工资薪金'],
      ...
      [['报销', '差旅', '补贴', '补助', '津贴', '交通补贴', '餐补',
        '话补', '房补', '高温补贴', '取暖补贴', '退款', '理赔'], '报销退款'],
      [['收到红包', '收红包', '份子钱收入', '礼金', '压岁钱',
        '家人给', '父母给', '老公转', '老婆转', '亲属给'], '红包礼金'],
      [['退税', '个税退税', '汇算清缴'], '其他收入', '退税'],
      [['政府补贴', '生育补贴', '稳岗补贴', '消费券'], '其他收入', '政府补贴'],
    ];
